...
def send_single_assignment_request(simulation_obj):
    
    try:
        debug_assignment("Processing single assignment request...")
        ...
        debug_assignment(f"Processing request for base {base_with_request.node_id}")
...
def periodic_assignment_request(env_var):
    ...
    while True:
        try:
            ...
            success = send_single_assignment_request(env_var)
            if success:
...
def generate_simulation_visualizations():
    ...
    if not os.path.exists('20_base_10000_taxi'):
        os.makedirs('20_base_10000_taxi')
    ...
    plt.tight_layout()
    plt.savefig('20_base_10000_taxi/hourly_order_statistics.png', dpi=300, bbox_inches='tight', 
    ...
    plt.tight_layout()
    plt.savefig('20_base_10000_taxi/base_order_performance.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.close()
    ...
    plt.ylim(0, 110)
    plt.tight_layout()
    plt.savefig('20_base_10000_taxi/service_rate_by_base.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.close()
