...
def send_broadcast_assignment_request(simulation_obj):
    try:
        debug_assignment("Processing broadcast assignment request...")
        base_with_request = None
        eligible_bases = [base for base in simulation_obj.all_bases if base.base_fleet.count(None) > 0]

        if eligible_bases:
            base_with_request = random.choice(eligible_bases)
        else:
            base_with_request = None

        if not base_with_request:
            debug_assignment("No bases need taxis")
            return False

        debug_assignment(f"Processing requests for base {base_with_request.node_id}")
        ...
        if not available_taxis:
            debug_assignment(f"No available taxis for base {base_with_request.node_id}")
            return False
        

        available_slots = base_with_request.base_fleet.count(None)
        assignments_made = 0

        debug_assignment(f"Base {base_with_request.node_id} has {available_slots} available slots, {len(available_taxis)} taxis available")
        for slot in range(min(available_slots, len(available_taxis))):
            closest_taxi = None
            ...
            for taxi in available_taxis:
                ...
                distance = geodesic(
                    (taxi.current_lat, taxi.current_lon),
                    (base_with_request.latitude, base_with_request.longitude)
                ).kilometers
            ...
            if closest_taxi:
                debug_assignment(f"Assigning taxi {closest_taxi.id} to base {base_with_request.node_id} (distance: {min_distance:.2f}km)")
                source_node = ox.nearest_nodes(G, X=closest_taxi.current_lon, Y=closest_taxi.current_lat)
                target_node = ox.nearest_nodes(G, X=base_with_request.longitude, Y=base_with_request.latitude)
                try:
                    route_node_id = nx.shortest_path(G, source=source_node, target=target_node, weight='duration')
                except Exception as e:
                    route_node_id = None
                assignment_data = {
                    "taxi_id": closest_taxi.id,
                    "assigned_base": str(base_with_request.node_id),
                    "encoded_route_node_id": route_node_id
                }
                closest_taxi.handle_assign_base(assignment_data)
                assignments_made +=1
                available_taxis.remove(closest_taxi)  
            else:
                debug_assignment(f"No suitable taxi found for base {base_with_request.node_id}")
        if assignments_made > 0:
            debug_assignment(f"Successfully made {assignments_made} assignments to base {base_with_request.node_id}")
            return True
        else:
            debug_assignment(f"No assignments could be made to base {base_with_request.node_id}")
            return False
    except Exception as e:
        debug_error(f"Single assignment failed: {e}")
        return False
...
def periodic_assignment_request(env_var):
    ...
    while True:
        try:
            ...
            success = send_broadcast_assignment_request(env_var)
            if success:
        ...
        yield env_var.timer.timeout(0.8)
...
def generate_simulation_visualizations():
    ...
    if assignment_stats['travel_times']:
        plt.hist(assignment_stats['travel_times'], bins=20, alpha=0.7, color='skyblue', edgecolor='black', linewidth=1.5)
        plt.xlabel('Duration (minutes)', fontsize=16, fontweight='bold')
        plt.ylabel('Frequency', fontsize=16, fontweight='bold')
        plt.title('Duration Distribution of Successful Orders', fontsize=18, fontweight='bold', pad=20)
        plt.grid(True, alpha=0.3)
    ...
    if assignment_stats['travel_distances']:
        plt.hist(assignment_stats['travel_distances'], bins=20, alpha=0.7, color='lightgreen', edgecolor='black', linewidth=1.5)
        plt.xlabel('Distance (km)', fontsize=16, fontweight='bold')
        plt.ylabel('Frequency', fontsize=16, fontweight='bold')
        plt.title('Distance Distribution of Successful Orders', fontsize=18, fontweight='bold', pad=20)
        plt.grid(True, alpha=0.3)
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
    else:
        plt.text(0.5, 0.5, 'No distance data available', ha='center', va='center', 
    ...
    bars = plt.bar(hours, hourly_stats['assignments_made'], alpha=0.7, color='purple', edgecolor='black', linewidth=1)
    plt.xlabel('Hour of Day', fontsize=16, fontweight='bold')
    plt.ylabel('Number of Assignments', fontsize=16, fontweight='bold')
    plt.title('Hourly Assignment Distribution', fontsize=18, fontweight='bold', pad=20)
    plt.grid(True, alpha=0.3, axis='y')
    plt.xticks(range(0, 24, 2), fontsize=14)
    plt.yticks(fontsize=14)
    ...
    for i in range(24):
        appeared = hourly_stats['orders_appeared'][i]
        served = hourly_stats['orders_served'][i]
        if appeared > 0:
            rate = (served / appeared) * 100
        else:
            rate = 0
        hourly_success_rates.append(rate)
    
    plt.plot(hours, hourly_success_rates, 'o-', linewidth=3, color='darkgreen', markersize=8)
    plt.xlabel('Hour of Day', fontsize=16, fontweight='bold')
    plt.ylabel('Success Rate (%)', fontsize=16, fontweight='bold')
    plt.title('Hourly Success Rate', fontsize=18, fontweight='bold', pad=20)
    plt.grid(True, alpha=0.3)
    plt.xticks(range(0, 24, 2), fontsize=14)
    plt.yticks(fontsize=14)
    plt.ylim(0, 105)
    ...
    if assignment_stats['travel_times'] and assignment_stats['travel_distances']:
        plt.scatter(assignment_stats['travel_distances'], 
                   assignment_stats['travel_times'], 
                   alpha=0.6, color='coral', s=60, edgecolors='black', linewidth=0.5)
        plt.xlabel('Distance (km)', fontsize=16, fontweight='bold')
        plt.ylabel('Duration (minutes)', fontsize=16, fontweight='bold')
        plt.title('Travel Time vs Distance', fontsize=18, fontweight='bold', pad=20)
        plt.grid(True, alpha=0.3)
        plt.xticks(fontsize=14)
        plt.yticks(fontsize=14)
    else:
        plt.text(0.5, 0.5, 'No travel data available', ha='center', va='center', 
    ...
    total_orders = sum(hourly_stats['orders_appeared'])
    total_served = sum(hourly_stats['orders_served'])
    total_failed = sum(hourly_stats['orders_failed'])
    overall_success_rate = (total_served / total_orders * 100) if total_orders > 0 else 0
    
    avg_duration = np.mean(assignment_stats['travel_times']) if assignment_stats['travel_times'] else 0
    avg_distance = np.mean(assignment_stats['travel_distances']) if assignment_stats['travel_distances'] else 0
...
Orders Failed: {total_failed:,}
Success Rate: {overall_success_rate:.1f}%

Average Duration: {avg_duration:.1f} min
Average Distance: {avg_distance:.1f} km

Total Assignments: {assignment_stats['total_assignments']:,}
Assignment Success: {assignment_stats['successful_arrivals']:,}

Simulation Time: {simulation_object.timer.current_time:.1f} units
Active Taxis: 1000
Active Bases: {len(base_data)}

Peak Hour Orders: {max(hourly_stats['orders_appeared'])}
Peak Assignment Hour: {max(hourly_stats['assignments_made'])}
"""
    
    plt.text(0.05, 0.95, summary_text, transform=ax9.transAxes, fontsize=14, 
    ...
    plt.savefig('20_base_10000_taxi.png', dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none')
    plt.show()
    
    print(" Comprehensive visualization saved as '20_base_10000_taxi.png'")
    print(" All graphs generated with improved size and clarity!")
...
def print_simulation_summary():
    ...
    print(f"Number of Taxis: 100")
    print(f"Number of Bases: {len(base_data)}")
    print(f"Number of Areas: 20")
    print("-"*80)
    ...
    if assignment_stats['travel_times']:
        avg_travel_time = sum(assignment_stats['travel_times']) / len(assignment_stats['travel_times'])
        min_travel_time = min(assignment_stats['travel_times'])
        max_travel_time = max(assignment_stats['travel_times'])
        
        print("TRAVEL TIME STATISTICS (Successful Arrivals Only):")
        print(f"  Average Travel Time: {avg_travel_time:.2f} simulation time units ({avg_travel_time:.2f} minutes)")
        print(f"  Minimum Travel Time: {min_travel_time:.2f} simulation time units ({min_travel_time:.2f} minutes)")
        print(f"  Maximum Travel Time: {max_travel_time:.2f} simulation time units ({max_travel_time:.2f} minutes)")
    else:
        print("TRAVEL TIME STATISTICS: No successful arrivals recorded")
    
    print("-"*80)
    
    if assignment_stats['travel_distances']:
        avg_distance = sum(assignment_stats['travel_distances']) / len(assignment_stats['travel_distances'])
        min_distance = min(assignment_stats['travel_distances'])
        max_distance = max(assignment_stats['travel_distances'])
        total_distance = sum(assignment_stats['travel_distances'])
        
        print("TRAVEL DISTANCE STATISTICS (Successful Arrivals Only):")
        ...
        print("TRAVEL DISTANCE STATISTICS: No successful arrivals recorded")
    
    print("-"*80)
    
    if assignment_stats['server_response_times']:
        avg_response_time = sum(assignment_stats['server_response_times']) / len(assignment_stats['server_response_times'])
        min_response_time = min(assignment_stats['server_response_times'])
        max_response_time = max(assignment_stats['server_response_times'])
        
        print("SERVER RESPONSE TIME STATISTICS:")
        ...
        print(f"  Total Requests: {len(assignment_stats['server_response_times'])}")
    else:
        print("SERVER RESPONSE TIME STATISTICS: No requests recorded")
    ...
    if sample_edges:
        print("TRAFFIC CONGESTION SAMPLE (First 10 Edges):")
        for u, v, data in sample_edges:
            congestion = data.get('congestion_level', 0)
            base_speed = data.get('speed_kph', 30)
            congested_speed = data.get('speed_kph_congested', base_speed)
            print(f"  Edge {u}-{v}: Congestion={congestion:.1f}%, Speed={base_speed:.1f}→{congested_speed:.1f} km/h")
